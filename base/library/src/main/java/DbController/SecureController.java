...
import main.java.CsvHandler.CsvReader;
...
public class SecureController implements Controller {
    ...
    private CompletableFuture<Boolean> lastInsertion; //changed from void to beeloean, ask pierre
    ...
    private CompletableFuture<Boolean> insertToDb(String key, String value) {
        CompletableFuture<Boolean> ret = CompletableFuture.completedFuture(null);
        try {
    ...
    public CompletableFuture<Boolean> insertEntry(String key, String value) {
    ...
    public CompletableFuture<String> getValue(String key) {
        ...
        try {
            ret = db.get(key.getBytes()).thenApply(x-> {
                if (x.isPresent())
